...
def start_analysis():
    ...
    for iteration in range(3):
        for i in range(links_count, len(links_with_text)):
            ...
            if not refine_str(links_with_text[i][1]).__eq__("0"):
                soup = BeautifulSoup(requests.get(refine_str(
                    links_with_text[i][1])).content, "html.parser")
                    ...
                    if count < limit:
                        ...
                        if not href is None:
                            links_with_text.append(
                                [refine_str(links_with_text[i][1]), href])
                            count += 1
    ...
    G = nx.from_pandas_edgelist(
        df, 'from', 'to', create_using=nx.DiGraph(length=50))
    ranks = dict(sorted(nx.pagerank(G).items(),
    ...
    d = dict(G.degree)
    nx.draw(G, with_labels=False, node_size=[
            d[k] * 10 for k in d], node_color="red", alpha=0.4, arrows=True, pos=nx.spring_layout(G))
    plt.show()
...
button_for_analyzing = tkinter.Button(
    master, text='Start', command=start_analysis)
