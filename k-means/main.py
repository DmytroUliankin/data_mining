...
if __name__ == '__main__':
    ...
    def choose_file():
        ...
        with open(filename, 'r') as read_file:
            ...
            for line in lines:
                ...
                points[coordinates_string[0].strip(
                )] = coordinates_string[1].strip()
    ...
    def run_method():
        ...
        points_dictionary = pd.read_csv(
            'points.csv', index_col=0, squeeze=True).to_dict()
        ...
        for key, value in points_dictionary.items():
            d = math.sqrt(math.pow((int(key) - list(centroids.keys())
                          [0]), 2) + math.pow((int(value) - list(centroids.values())[0]), 2))
            index_of_cluster = 0
            for index in range(1, int(number_of_clusters.get())):
                d1 = math.sqrt(math.pow((int(key) - list(centroids.keys())[index]), 2) + math.pow(
                    (int(value) - list(centroids.values())[index]), 2))
                if d1 < d:
            ...
            average_x = sums_x[index_of_cluster] / \
                len(array_of_lists_of_points[index_of_cluster])
            average_y = sums_y[index_of_cluster] / \
                len(array_of_lists_of_points[index_of_cluster])
            centroids[list(centroids.keys())[index_of_cluster]] = average_x
            centroids[average_y] = centroids[list(centroids.keys())[
                index_of_cluster]]
            del centroids[list(centroids.keys())[index_of_cluster]]

        for i in range(int(number_of_clusters.get())):
            array_of_lists_of_points[i] = {int(k): int(
                v) for k, v in array_of_lists_of_points[i].items()}
            ax1.scatter(array_of_lists_of_points[i].keys(
            ), array_of_lists_of_points[i].values(), color=colors[i])
    ...
    number_of_clusters_entry = ttk.Spinbox(
        master, from_=1, to=1000, textvariable=number_of_clusters, wrap=False)

    # buttons
    button_for_file = tkinter.Button(
        master, text='Choose', command=choose_file)
